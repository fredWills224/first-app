        ...
        // get all the [checks]
            _data.list('checks', function(err, checks){
                ...
                if(!err && checks && checks.length > 0){
                    ...
                    debug("Error: Could not find any cheks to process");
                }
...
// [rotateLogs] rotate (compress) the log files in [logs] array
    workers.rotateLogs = function(){
        // List all the (non compressed) log files in [logs]
            // [_logs.list].firstParameter -> boolean -> [includeCompressedLogs]
                _logs.list(false, function(err, logs){

                    if(!err && logs && logs.length > 0){

                        logs.forEach(function(logName){
                            // Compress the [logName].data to a different file using [newFileId]
...
// Init script
    workers.init = function(){
        // Send to console, in yellow
            // modify [33] in ['\x1b[33m%s\x1b[0m'] variable to change color
                /*[31(red), 32(green), 33(yellow), 34(navyblue), 35(purple), 36(skyblue)]*/
